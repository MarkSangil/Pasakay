// Sends FCM to passenger devices registered in commuter_device_tokens.
//
// Secrets (Edge Functions → Secrets):
//   FIREBASE_PASSENGER_SERVICE_ACCOUNT_JSON = service account JSON from
//   Firebase project `pasakay-passenger` (separate from the driver project).
//   NOTIFICATION_PUSH_SECRET = shared secret used by the DB pg_net trigger
//
// Auth: Bearer SUPABASE_SERVICE_ROLE_KEY or NOTIFICATION_PUSH_SECRET.
// Manual: { "commuter_id": "<uuid>", "title": "Pasakay", "body": "Hello" }
...
Deno.serve(async (req) => {
  ...
  const saRaw = Deno.env.get("FIREBASE_PASSENGER_SERVICE_ACCOUNT_JSON");
  if (!saRaw) {
    return json({
      error:
        "Missing FIREBASE_PASSENGER_SERVICE_ACCOUNT_JSON secret (pasakay-passenger Firebase service account).",
    }, 500);
  }
  ...
  try {
    ...
    return json({ error: "FIREBASE_PASSENGER_SERVICE_ACCOUNT_JSON is not valid JSON" }, 500);
  }
  ...
  let commuterId = payload.commuter_id ?? payload.record?.recipient_id;
  ...
  if (payload.record?.recipient_type && payload.record.recipient_type !== "commuter") {
    return json({ ok: true, skipped: "not a passenger notification" });
  }

  if ((!commuterId || !body) && notificationId) {
    ...
    if (data.recipient_type !== "commuter") {
      return json({ ok: true, skipped: "not a passenger notification" });
    }
    commuterId = data.recipient_id as string;
  ...
  if (!commuterId || !body) {
    return json({ error: "commuter_id and body (or notification_id) are required" }, 400);
  }
      ...
      body: JSON.stringify({
        message: {
          ...
          android: {
            priority: "HIGH",
            notification: {
              channel_id: "pasakay_passenger_alerts",
